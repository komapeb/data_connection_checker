...
void main() async {
  group('data_connection_checker', () {
    ...
    test('''Initial results List 'lastTryResults' should be empty''', () {
      expect(DataConnectionChecker().lastTryResults, isEmpty);
    });

    test('''Shouldn't have any listeners attached''', () {
      expect(DataConnectionChecker().hasListeners, isFalse);
    });

    test('''Unawaited call hasConnection should return a Future<bool>''', () {
      expect(DataConnectionChecker().hasConnection, isA<Future<bool>>());
    });

    test('''Awaited call to hasConnection should return a bool''', () async {
      expect(await DataConnectionChecker().hasConnection, isA<bool>());
    });
    ...
    test(
      ...
      expect(DataConnectionChecker().lastTryResults, isNotEmpty);
    });

    test(
        '''Unawaited call to connectionStatus '''
        '''should return a Future<DataConnectionStatus>''', () {
      expect(DataConnectionChecker().connectionStatus,
    ...
    test(
      ...
      expect(await DataConnectionChecker().connectionStatus,
    ...
    test('''We should have listeners 1''', () {
      ...
      expect(DataConnectionChecker().hasListeners, isTrue);
    });
